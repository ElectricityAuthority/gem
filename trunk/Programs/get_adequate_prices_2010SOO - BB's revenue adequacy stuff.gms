...
option limcol = 0, limrow = 0, reslim = 50000, iterlim = 1000000 ;
option sysout = on, solprint = off ;

SETS
  REQUIRE_ADEQUACY_PEAK(k)    'Peaking technologies for which we assert adequacy'  / BioPkr,GasPkr,DslPkr,HydPD,DslRecip /
  REQUIRE_ADEQUACY_MID(k)     'Mid-order technologies for which we assert adequacy'  / Coal,Lig,IGCC,IGCC_CCS,IGCC_Lig,IGCC_LigCCS,CCGT,HydPk,CCGT_CCS /
  REQUIRE_ADEQUACY_BASE(k)    'Baseload, intermittent and process-driven technologies for which we assert adequacy'  / Wave,Tide,OthCog,BioCog,CoalCog,GasCog,Geo,HydRR,Wind,Solar /
  ;
...
$gdxin 'Draft SOO 8 - PreparedOutput.gdx';
$load numdisyrs 

$gdxin 'all.gdx';
$load s_gen_2=s_gen s_capacity_2=s_capacity

s_genyr_2(mds,rt,g,hYr,y) = SUM((t,lb,hd)$(sameas(hd,'dum')),s_gen_2(mds,rt,hYr,g,y,t,lb,hd));

$gdxin 'Draft SOO 8 - miscout.gdx';
$load s_solveindex s_hdindex 
