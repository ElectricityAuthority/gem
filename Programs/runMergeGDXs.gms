...
*    - merge all runVersion name GDX files into a single GDX file called 'allRV_SelectedInputData.gdx'.
putclose bat
  'del mds1.gdx /q' /
  'del mds2.gdx /q' /
  'del mds3.gdx /q' /
...
*    - merge all runVersion name GDX files into a single GDX file called 'allRV_ReportOutput.gdx'.
putclose bat
  'del mds1.gdx /q' /
  'del mds2.gdx /q' /
  'del mds3.gdx /q' /
...
*    - delete any GDX files that go by the name of the elements in runVersions.
*    - copy each allExperimentsAllOutput GDX file to a GDX file that goes by its corresponding runVersion name, and
*    - merge all runVersion name GDX files into a single GDX file called 'allRV_allOutput.gdx'.
putclose bat
  'del mds1.gdx /q' /
  'del mds2.gdx /q' /
  'del mds3.gdx /q' /
  'del mds4.gdx /q' /
  'del mds5.gdx /q' /
  'copy "%OutPath%\%runName%\GDX\allExperimentsAllOutput - %runName%_mds1.gdx" mds1.gdx' /
  'copy "%OutPath%\%runName%\GDX\allExperimentsAllOutput - %runName%_mds2.gdx" mds2.gdx' /
  'copy "%OutPath%\%runName%\GDX\allExperimentsAllOutput - %runName%_mds3.gdx" mds3.gdx' /
  'copy "%OutPath%\%runName%\GDX\allExperimentsAllOutput - %runName%_mds4.gdx" mds4.gdx' /
  'copy "%OutPath%\%runName%\GDX\allExperimentsAllOutput - %runName%_mds5.gdx" mds5.gdx' /
  ;
execute 'temp.bat' ;
execute 'gdxmerge mds1.gdx mds2.gdx mds3.gdx mds4.gdx mds5.gdx output="allRV_allOutput_%runName%.gdx" big=100000'
...
putclose bat
  ...
  'copy "allRV_allOutput_%runName%.gdx"         "%OutPath%\%runName%\GDX\"' / ;

execute 'temp.bat' ;


* 5. Create a progress report file indicating that runMergeGDXs is now finished.
